...
class SumTree(object):
	...
	def _get_priority(self, error):
		np.power(error)/self.tree[root]

	def _get_leaf(self, lower_boundery):
		idx = _retrieve(lower_boundary)
		p = self.tree[idx]
		data = self.data[idx]
		return data
    ...
    def _propagate_change(self, tree_idx, change):
        ...
        if parent_idx != 0:
            self.propagate_change(parent_idx, change)
    ...
    def _retrieve(self, lower_bound):
...
class Memory(object):
    ...
    def store(self, error, transition):
        ...
        self.tree_add_new_priority(p, transition)
    ...
    def sample(self, n):
        ...
        min_prob = np.min(self.tree.tree[-self.tree.capacity:]) / sefl.tree.root_priority
...
class DuelingDQNPrioritizedReplay:
    ...
    def _replace_target_params(self):
        ...
        e_params = tf.get_collection['eval_net_params11']
    ...
    def learn(self):
        ...
        batch_index = np.arrange(self.batch_size, dtype=np.size)
        ...
        for i in range(len(tree_idx)):
            idx = tree_idx[i]
            self.memory.update(idx, abs_erros[i])

        self.cost_his_append(self.cost)
